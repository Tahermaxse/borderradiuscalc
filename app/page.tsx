...
import { ZoomIn, ZoomOut, Sun, Moon, Copy, Check, Github } from "lucide-react";
...
.inner-box {
  ...
  return (
    <div
      className={`overflow-hidden min-h-screen p-4 md:p-6 ${
        theme === "dark" ? "bg-[#131313] text-white" : "bg-gray-100 text-black"
      }`}
    >
      <div className="max-w-8xl mx-auto grid grid-cols-1 lg:grid-cols-[1fr,300px] gap-4 md:gap-6">
        {/* Playground Area */}
        <div
          className={`relative h-[585px] overflow-hidden backdrop-blur rounded-2xl p-4 md:p-6 ${
        ...
        >
          ...
          <div className="flex justify-start mb-6  md:mb-8">
            <div className="flex items-center gap-3 text-sm">
              <span className="text-white bg-[#343537] px-2 py-1 rounded-md">
                R1
              </span>
          ...
          <div
            className="relative aspect-square max-w-2xl max-h-[450px] mx-auto mt-[5rem]"
          ...
          >
            ...
            {zoom >= 1.5 && (
              <>
                <span className="absolute top-0 left-1/5 text-sm text-[#6366f1]">
                  {outerRadius}px
                </span>
                <span className="absolute top-[45px] left-1/4 text-sm text-[#ec4899]">
                  {innerRadius}px
                </span>
                <span className="absolute top-[20px] left-[65px] text-sm text-zinc-400">
                  {padding}px
                </span>
              </>
            )}
            ...
            <TooltipProvider delayDuration={0}>
              <Tooltip>
                <TooltipTrigger asChild>
                  <Link href="https://github.com/Tahermaxse/borderradiuscalc.git">
                  <Button
                    variant="ghost"
                    size="icon"
                    className="h-8 w-8 dark:hover:bg-zinc-800"
        ...
        {/* Controls Sidebar */}
        <div
          className={`backdrop-blur rounded-2xl p-4 md:p-6 space-y-6 md:space-y-8 ${
            theme === "dark" ? "bg-[#1f1f21]" : "bg-white/50"
          }`}
        >
          ...
          <div className="pt-4">
            ...
            <pre
              className={`p-4 rounded-bl-lg rounded-br-lg text-xs ${
                theme === "dark" ? "bg-[#292a2c]" : "bg-gray-100"
              }`}
            >
